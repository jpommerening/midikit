...
int test001_message_format( void ) {
  ...
  ASSERT_NO_ERROR( (format->decode)( message, sizeof(buffer), &buffer[0] ), "Could not decode note off buffer." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_STATUS, sizeof(MIDIStatus), &status ), "Could not get message status." );
  ASSERT_EQUAL( status, MIDI_STATUS_NOTE_OFF, "Stored wrong status." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_CHANNEL, sizeof(MIDIChannel), &channel ) , "Could not get message channel." );
  ASSERT_EQUAL( channel, MIDI_CHANNEL_1, "Stored wrong channel." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_KEY, sizeof(MIDIKey), &key ), "Could not get message key." );
  ASSERT_EQUAL( key, 123, "Stored wrong key." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_VELOCITY, sizeof(MIDIVelocity), &velocity ), "Could not get message velocity." );  
...
int test002_message_format( void ) {
  ...
  ASSERT_NO_ERROR( (format->decode)( message, sizeof(buffer), &buffer[0] ), "Could not decode pitch wheel change buffer." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_STATUS, sizeof(MIDIStatus), &status ), "Could not get message status." );
  ASSERT_EQUAL( status, MIDI_STATUS_PITCH_WHEEL_CHANGE, "Stored wrong status." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_CHANNEL, sizeof(MIDIChannel), &channel ) , "Could not get message channel." );
  ASSERT_EQUAL( channel, MIDI_CHANNEL_1, "Stored wrong channel." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_VALUE_LSB, sizeof(MIDIValue), &lsb ), "Could not get message value lsb." );
  ASSERT_EQUAL( lsb, MIDI_LSB(12345), "Stored wrong value lsb." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_VALUE_MSB, sizeof(MIDIValue), &msb ), "Could not get message value msb." );
  ASSERT_EQUAL( msb, MIDI_MSB(12345), "Stored wrong value msb." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_VALUE, sizeof(MIDILongValue), &value ), "Could not get message value." );  
...
int test003_message_format( void ) {
  ...
  ASSERT_NO_ERROR( (format->decode)( message, sizeof(buffer), &buffer[0] ), "Could not decode system exclusive buffer." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_STATUS, sizeof(MIDIStatus), &status ), "Could not get message status." );
  ASSERT_EQUAL( status, MIDI_STATUS_SYSTEM_EXCLUSIVE, "Stored wrong status." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_MANUFACTURER_ID, sizeof(MIDIManufacturerId), &manufacturer_id ) , "Could not get message manufacturer id." );
  ASSERT_EQUAL( manufacturer_id, 123, "Stored wrong manufacturer id." );
  ASSERT_NO_ERROR( (format->get)( message, MIDI_SYSEX_DATA, sizeof(sysex_data), &sysex_data[0] ), "Could not get mssage system exclusive data." );
  for( i=0; i<12; i++ ) {
